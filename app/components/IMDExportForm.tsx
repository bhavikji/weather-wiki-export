...
export default function IMDExportForm() {
  // Controlled inputs
  ...
  // O(1) option lookup
  const options = IMD_SHEET_OPTIONS;
  ...
  const stationId = (selected?.station_id ?? "").toString().trim();
  const isDateValid = useMemo(() => isISODate(dateText), [dateText]);

  const canSubmit =
    Boolean(sheetId) && Boolean(stationId) && isDateValid && !submitting;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/imd-to-sheets", {
        method: "POST",
      ...
      setResult({
        kind: "success",
        message: `IMD sync complete for ${data.date}. Wrote ${data.wroteCells} cell(s).`,
      });
    } catch (err) {
      setResult({
        kind: "error",
    ...
    <div className="mx-auto w-full max-w-3xl">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          Writes a single day’s IMD observations into the selected spreadsheet
          without clearing other rows.
        </p>
      </div>
      ...
      >
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">
              IMD “static” endpoint is typically current-day for the station.
              Use today’s date in most cases.
            </p>
          ...
          <div className="md:col-span-2 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">
                Read-only metadata used for IMD API.
              </p>
              <input
                value={stationId}
                readOnly
              ...
              {!stationId && sheetId ? (
                <div className="mt-2 text-xs text-red-600">
                  This option does not have a station_id. Please fix
                  IMD_SHEET_OPTIONS.
                </div>
              ) : null}
            </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            {submitting ? "Syncing…" : "Sync IMD Daily Data"}
          </button>
      ...
      <div className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 shadow-sm">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-zinc-600">
          ...
          <li>This page is IMD-only; formatting is handled by the template.</li>
        </ul>
      </div>
    </div>
  );
