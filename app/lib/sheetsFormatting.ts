...
import type { SheetCell } from "@/app/types/sheet-values.types";
...
// Keep display (Column A) stable across all years by formatting in UTC.
export function prettyDateFromIsoUtc(iso: string): string {
  return moment.utc(iso, "YYYY-MM-DD", true).format("Do MMM YYYY");
}
...
/**
 * Convert an OpenMeteoRow to ONLY the data cells (B..end).
 * Column A is generated by the UTC calendar template.
 */
export function toSheetDataCells(
  row: OpenMeteoRow,
  yearTimezone: string
): SheetCell[] {
